...
- Stop before human-gated or external actions.
...
- Build a run plan.
- Execute only safe draft/planning steps unless approval is present.
...
- Keep the SkillSpec `pending_approval`.
- Do not decompose or activate the skill until human approval is recorded.
- Never mark a generated skill as approved.

## Boundaries

- Do not send emails, create meetings, publish files, commit code, or call external systems when `requires_human` is true.
